...
public class ResponseTypeHandlerUtil {
    ...
    private static AccessTokenDO getExistingToken(OAuthAuthzReqMessageContext oauthAuthzMsgCtx, boolean cacheEnabled)
        ...
        String scope = OAuth2Util.buildScopeString(oauthAuthzMsgCtx.getApprovedScope());
        String consumerKey = authorizationReqDTO.getConsumerKey();
        String authorizedUser = OAuthUtil.getFullyQualifiedUserName(authorizationReqDTO.getUser());
            ...
            if (log.isDebugEnabled()) {
                if (IdentityUtil.isTokenLoggable(IdentityConstants.IdentityTokens.ACCESS_TOKEN)) {
                    log.debug("Retrieved active access token(hashed): " + DigestUtils.sha256Hex(existingTokenBean
                            .getAccessToken()) + " in state: " + existingTokenBean.getTokenState() + " for client Id:" +
                            " " + consumerKey + ", user: " + authorizedUser + " and scope: " + scope + " from cache.");
    ...
    private static AccessTokenDO getExistingTokenFromDB(OAuthAuthzReqMessageContext oauthAuthzMsgCtx, boolean
        ...
        if (existingToken != null) {
            ...
            if (TOKEN_STATE_ACTIVE.equals(existingToken.getTokenState()) && expireTime != 0 && cacheEnabled) {
                ...
                addTokenToCache(getOAuthCacheKey(
                        consumerKey, scope, OAuthUtil.getFullyQualifiedUserName(authorizedUser)), existingToken);
            }
        }
    ...
    private static void persistAccessTokenInDB(OAuthAuthzReqMessageContext oauthAuthzMsgCtx, AccessTokenDO
        ...
        if (log.isDebugEnabled()) {
            log.debug("Persisted Access Token for" +
                    ...
                    ", Is Federated User: " + authorizationReqDTO.getUser().isFederatedUser() +
                    ", Timestamp: " + newTokenBean.getIssuedTime() +
    ...
    private static long getAccessTokenExpiryTimeMillis(AccessTokenDO tokenBean) throws
            IdentityOAuth2Exception {
        ...
        if (log.isDebugEnabled()) {
            if (IdentityUtil.isTokenLoggable(IdentityConstants.IdentityTokens.ACCESS_TOKEN)) {
                if (expireTimeMillis > 0) {
                    ...
                    ) + " is still valid. Remaining time: " + expireTimeMillis + " ms");
                } else {
                ...
                if (expireTimeMillis > 0) {
                    ...
                    log.debug("Infinite lifetime Access Token found for client: " + tokenBean
                            .getConsumerKey());
                }
            }
        }
    ...
    private static void removeTokenFromCache(OAuthCacheKey cacheKey, AccessTokenDO tokenBean) {
        ...
        if (log.isDebugEnabled()) {
            if (IdentityUtil.isTokenLoggable(IdentityConstants.IdentityTokens.ACCESS_TOKEN)) {
                ...
                log.debug("Existing access token for client: " + tokenBean.getConsumerKey() + " is " +
                        "expired. Therefore cleared it from cache.");
            }
        }
    }
}
